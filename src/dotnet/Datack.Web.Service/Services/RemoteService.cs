...
namespace Datack.Web.Service.Services
{
    public class RemoteService
    {
        ...
        {
            return await Send<String>(server.Key, "TestSqlServer", server.DbSettings, cancellationToken);
        }

        public async Task<IList<Database>> GetDatabaseList(Server server, CancellationToken cancellationToken)
        {
            return await Send<List<Database>>(server.Key, "GetDatabaseList", null, cancellationToken);
        }

        private async Task<T> Send<T>(String key, String method, Object payload, CancellationToken cancellationToken)
        {

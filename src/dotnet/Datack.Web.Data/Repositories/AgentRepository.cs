...
public class AgentRepository(DataContext dataContext)
{
    ...
    public async Task<Guid> Add(Agent agent, CancellationToken cancellationToken)
    {
        ...
        await dataContext.Agents.AddAsync(agent, cancellationToken);
    ...
    {
        var dbAgent = await dataContext.Agents.FirstOrDefaultAsync(m => m.AgentId == agent.AgentId, cancellationToken);
    ...
    {
        var dbAgent = await dataContext.Agents.FirstOrDefaultAsync(m => m.AgentId == agentId, cancellationToken);
        ...
        dataContext.Agents.Remove(dbAgent);

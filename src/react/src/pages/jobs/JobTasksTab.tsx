...
const JobTasksTab: FC<JobTasksTabProps> = (props) => {
  ...
  return (
    <Loader isLoaded={isLoaded} error={error}>
            ...
            {(provided, snapshot) => (
              <Tbody {...getTableBodyProps()} {...provided.droppableProps} ref={provided.innerRef}>
                {rows.map((row) => {
                  prepareRow(row);
                  return (
                    <Draggable draggableId={row.original.jobTaskId} key={row.original.jobTaskId} index={row.index}>
                      {(provided, snapshot) => {
                        return (
                          <Tr {...row.getRowProps()} {...provided.draggableProps} ref={provided.innerRef}>
      ...
      <Button onClick={() => handleAddNewJobTaskClick()}>Add new task</Button>
    </Loader>
  );
};

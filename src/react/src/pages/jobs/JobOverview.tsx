...
const JobOverview: FC = () => {
  ...
  useEffect(() => {
    if (restoreDatabase) {
      (async () => {
        const result = await Jobs.getAzureFileList(params.id!, restoreDatabase, cancelToken);
  ...
  const handleRestoreOk = async () => {
    if (!restoreDatabase || !restoreFile) {
      return;
    }

    setShowRestoreModal(false);

    var jobRunId = await Jobs.run(job!.jobId, restoreFile);
